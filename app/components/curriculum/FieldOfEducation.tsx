...
export default function FieldOfEducation() {
  ...
  return (
    <section
      className="pt-[80px] pb-[120px] md:py-[80px] min-h-[860px] flex flex-col justify-center relative z-30 -mt-10"
      style={{
    ...
    >
      ...
      >
      <Container className="mt-0 md:-mt-20">
        <div className="pl-6 pr-6 md:pl-20 md:pr-20">
          ...
          <p className="text-[15px] text-[#5a6895] mt-3">
            디지털 전환 시대에 필요한 핵심 기술을 <br className="md:hidden" />
            교육합니다.
          </p>
        </div>
      </Container>
              ...
              {fields.map((field, i) => (
                ...
                >
                  <div className="group h-full flex flex-col items-center justify-start bg-white border border-[#E5E7EB] rounded-xl p-8 text-center transition-all duration-300 hover:bg-[#2563EB] hover:border-[#2563EB] hover:shadow-[0_10px_30px_rgba(37,99,235,0.3)]">
                    ...
                    <p className="text-[14px] text-[#6B7280] mt-2.5 leading-[1.7] transition-colors duration-300 group-hover:text-white/90">{renderBreaks(field.body, `field-${field.number}`)}</p>
                  </div>
                </div>
              ))}
            </div>
